...
class AdversarialTrainer(BaseTrainer):
    ...
    def __init__(self, model, loss, metrics, optimizer, config,
        ...
        if attack_params["ratio"] is not None:
            ...
            self.eps = attack_params["kwargs"]["eps_iter"]
        ...
        self.perturbation = torch.zeros(
            self.len_epoch,
            config['data']['loader']['args']['batch_size'],
            3, 32, 32)
    ...
    def _train_epoch(self, epoch):
        ...
        for batch_idx, (data, target) in \
            ...
            loss, adv_loss, metrics, _ = self._run_batch(
                batch_idx, data, target, eval_metrics=True)
            total_loss += loss
    ...
    # Epoch helpers
    ###
    def _run_batch(self, batch_id, data, target, eval_metrics=False,
                   train=True):
        ...
        original_data = data
        adversarial_data = self._get_adversarial_data(batch_id, data, target)
    ...
    def _get_adversarial_data(self, batch_id, data, target):
        """Returns adversarial data"""
        # TODO: add logic for warm-up
        with ctx_noparamgrad_and_eval(self.model):
            adversarial_data = self.adversary.perturb(data, target)
        pert = adversarial_data - data
        self._accumulate_perturbations(batch_id, pert)
    ...
    def _valid_epoch(self, epoch):
        ...
        for batch_idx, (data, target) in enumerate(self.valid_data_loader):
            ...
            loss, adv_loss, metrics, dic_metrics = self._run_batch(
                batch_idx, data, target, eval_metrics=True, train=False)
            total_val_loss += loss
    ...
    def _test_epoch(self, epoch):
        ...
        for i, (data, target) in enumerate(tqdm(self.test_data_loader)):
            ...
            loss, adv_loss, metrics, dic_metrics = self._run_batch(
                i, data, target, eval_metrics=True, train=False)
    ...
    def _accumulate_perturbations(self, batch_id, pert):
        """Accumulates perturbations for each batch"""
        self.perturbation[batch_id] += pert
        self.perturbation[batch_id]._clamp(-self.eps, self.eps)
